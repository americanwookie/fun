...
sub pulldown() 
{
    ...
    if(   exists( $this->{-menu}->[$this->{-selected}]->{-value} )
       && ref( $this->{-menu}->[$this->{-selected}]->{-value} ) eq 'CODE' ) {
        $this->{-menu}->[$this->{-selected}]->{-value}->();
    }

    if(   !exists( $this->{-menu}->[$this->{-selected}]->{-noexpand} ) 
       || !$this->{-menu}->[$this->{-selected}]->{-noexpand} ) {
        # Add the submenu.
        my $id = "__submenu_$this";
        my $submenu = $this->root->add(
            $id, 'MenuListbox',
